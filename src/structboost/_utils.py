...
#: Bytes of dense output materialized per row block by :func:`densify`. Large
#: enough that the per-block cost of slicing a CSR matrix is negligible against
#: the copy itself, small enough that the temporary never rivals the result.
_DENSIFY_BLOCK_BYTES = 32 * 1024**2
...
def densify(
    ...
    *,
    ...
    ``matrix.toarray().astype(dtype)`` holds the dense matrix twice at once: the
    source-dtype array returned by ``toarray`` and the converted copy. On a
    float64 CSR panel that peaks at ``12 * n * p`` bytes to produce a ``4 * n * p``
    byte result — 18 GB for 500,000 cells x 3,000 genes, which is where a fit
    stops being possible rather than merely slow. Converting in row blocks
    straight into the output removes the intermediate entirely: the peak becomes
    the result plus one block.

    **The bytes produced are identical to the unchunked expression.** ``toarray``
    scatters stored values into a zero-filled buffer and the ``dtype`` conversion
    is elementwise, so neither depends on how the rows are grouped. No arithmetic
    is reassociated, and nothing here rounds differently.

    Copies only when it must, like :func:`numpy.asarray`: an ndarray that already
    has ``dtype`` is returned as-is. Callers use the result read-only and let it
    go out of scope, so the array is never aliased into a fitted model.

    Parameters
    ----------
    matrix
        Dense array-like, or any SciPy sparse matrix. Non-CSR sparse formats are
        converted to CSR first, because row slicing is what the block loop needs
        and doing it per block on a column-major format costs ``O(nnz)`` each
    ...
    source = matrix if matrix.format == "csr" else matrix.tocsr()
    rows_per_block = max(1, block_bytes // (n_cols * dtype.itemsize))
    for start in range(0, n_rows, rows_per_block):
        stop = min(start + rows_per_block, n_rows)
        out[start:stop] = source[start:stop].toarray()
    return out
